...
Closure::Closure(const Cosmology& C_, const PowerSpectrum& P_i_, real z_i_, real z_, int Neta_, int Nk_, real kcut_)
    : C(C_), P_i(P_i_), p3int(*this)
{
    ...
    MonteCarloIntegral::Range domain[4] = {
        { log(QMIN), log(kcut) },
        { log(QMIN), log(kcut) },
        { -1, 1 },
        { -1, 1 }
...
/* k-dependent factors */
real Closure::G_f(real k) const {
    return k*k/(4*M_PI*M_PI) * 1/252. * Integrate<ExpSub>(bind(B_f, cref(P_i), k, std::placeholders::_1), QMIN, QMAX, EPS);
}
real Closure::G_g(real k) const {
    return k*k/(4*M_PI*M_PI) * 1/84. * Integrate<ExpSub>(bind(B_g, cref(P_i), k, std::placeholders::_1), QMIN, QMAX, EPS);
}
real Closure::G_h(real k) const {
    return k*k/(4*M_PI*M_PI) * 1/12. * Integrate<ExpSub>(bind(B_h, cref(P_i), k, std::placeholders::_1), QMIN, QMAX, EPS);
}
real Closure::G_i(real k) const {
    return -k*k/(4*M_PI*M_PI) * 1/36. * Integrate<ExpSub>(bind(B_i, cref(P_i), k, std::placeholders::_1), QMIN, QMAX, EPS);
}

void Closure::ComputeG_ab(real k, real eta1, real eta2, real& g11, real& g12, real& g21, real& g22) const {
    real eta = eta1 - eta2;
...
void Closure::PrecomputePropagator() {
...
real Closure::G_11(real k, int t, int tp) const {
    if(t == -1)
        t += Neta;
    return g11[Neta*t + tp](k);
}

real Closure::G_12(real k, int t, int tp) const {
    if(t == -1)
        t += Neta;
    return g12[Neta*t + tp](k);
}

real Closure::G_21(real k, int t, int tp) const {
    if(t == -1)
        t += Neta;
    return g21[Neta*t + tp](k);
}

real Closure::G_22(real k, int t, int tp) const {
    if(t == -1)
        t += Neta;
    return g22[Neta*t + tp](k);
}

real Closure::G_1(real k, int t) const {
    return G_11(k, t) + G_12(k, t);
}

real Closure::G_2(real k, int t) const {
...
real Closure::I_1(real k, real q, real kq, int t) const {
    if(t == -1)
        t += Neta;
...
real Closure::I_2(real k, real q, real kq, int t) const {
    if(t == -1)
        t += Neta;
    real gamma112 = alpha(k, kq, q)/2;
    real gamma121 = alpha(k, q, kq)/2;
...
real Closure::J_1(real k, real p, real kp, real q, real pq, int t) const {
    if(t == -1)
        t += Neta;
...
real Closure::J_2(real k, real p, real kp, real q, real pq, int t) const {
    if(t == -1)
        t += Neta;
    real gamma112 = alpha(k, kp, p)/2;
    real gamma121 = alpha(k, p, kp)/2;
...
real Closure::P1_11(real k) const {
    return (k == 0) ? 0 : pow2(G_1(k)) * P_i(k);
}
real Closure::P1_12(real k) const {
    return (k == 0) ? 0 : G_1(k)*G_2(k) * P_i(k);
}
real Closure::P1_22(real k) const {
    return (k == 0) ? 0 : pow2(G_2(k)) * P_i(k);
}

real Closure::P1(real k, int a, int b) const {
    switch(a*b) {
        case 1:
            return P1_11(k);
        case 2:
        ...
        case 4:
            return P1_22(k);
        default:
            warning("Closure: invalid indices, a = %d, b = %d\n", a, b);
            return 0;
    }
}
...
real Closure::P2_11(real k) const {
#if 0
    real a[2] = { QMIN, -1 };
    real b[2] = { kcut, +1 };
    return (k == 0) ? 0 : 2*Integrate<2>(bind(&Closure::f2_11, cref(*this), k, std::placeholders::_1, std::placeholders::_2), a, b, 1e-3, 1e-3*P_L(k));
#endif
    real a[2] = { k/M_SQRT2, 0 };
    real b[2] = { kcut, k/M_SQRT2 };
    return (k == 0) ? 0 : 2*Integrate<2>(bind(&Closure::F2_11, cref(*this), k, std::placeholders::_1, std::placeholders::_2), a, b, 1e-3, 1e-3*P_L(k)/2);
}
real Closure::P2_12(real k) const {
#if 0
    real a[2] = { QMIN, -1 };
    real b[2] = { kcut, +1 };
    return (k == 0) ? 0 : 2*Integrate<2>(bind(&Closure::f2_12, cref(*this), k, std::placeholders::_1, std::placeholders::_2), a, b, 1e-4, 1e-4*P_L(k));
#endif
    real a[2] = { k/M_SQRT2, 0 };
    real b[2] = { kcut, k/M_SQRT2 };
    return (k == 0) ? 0 : 2*Integrate<2>(bind(&Closure::F2_12, cref(*this), k, std::placeholders::_1, std::placeholders::_2), a, b, 1e-3, 1e-3*P_L(k)/2);
}
real Closure::P2_22(real k) const {
#if 0
    real a[2] = { QMIN, -1 };
    real b[2] = { kcut, +1 };
    return (k == 0) ? 0 : 2*Integrate<2>(bind(&Closure::f2_22, cref(*this), k, std::placeholders::_1, std::placeholders::_2), a, b, 1e-4, 1e-4*P_L(k));
#endif
    real a[2] = { k/M_SQRT2, 0 };
    real b[2] = { kcut, k/M_SQRT2 };
    return (k == 0) ? 0 : 2*Integrate<2>(bind(&Closure::F2_22, cref(*this), k, std::placeholders::_1, std::placeholders::_2), a, b, 1e-3, 1e-3*P_L(k)/2);
}

real Closure::P2(real k, int a, int b) const {
    switch(a*b) {
        case 1:
            return P2_11(k);
        case 2:
...
real Closure::P3_11(real k) const {
    if(k == 0)
        return 0;
...
real Closure::P3_12(real k) const {
    if(k == 0)
        return 0;

    warning("Closure::P3_12 not implemented yet\n");
    return 0;
}

real Closure::P3_22(real k) const {
    if(k == 0)
        return 0;

    warning("Closure::P3_22 not implemented yet\n");
    return 0;
}

real Closure::P3(real k, int a, int b) const {
    switch(a*b) {
        case 1:
            return P3_11(k);
        case 2:
...
Closure::ThirdOrderIntegral::ThirdOrderIntegral(const Closure& closure_)
    : MonteCarloIntegral(4), closure(closure_)
{
    maxeval = 1000000;
}

void Closure::ThirdOrderIntegral::Integrand(const double x[], double* f, double* param) const {
    real k = *param;
    real logp = x[0];
    real logq = x[1];
    real mu_p = x[2];
    ...
    real pq = sqrt(p*p + q*q - 2*p*q*mu_q);
    f[0] = 8*pow3(p)*pow3(q)/pow4(2*M_PI) * pow2(closure.J_1(k,p,kp,q,pq)) * closure.P_i(kp)*closure.P_i(q)*closure.P_i(pq);
}

real Closure::P_L(real k) const {
