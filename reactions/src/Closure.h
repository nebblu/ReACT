 ...
 * Closure
 *
 * Taruya and Hiramatsu's closure theory
 ******************************************************************************/
class Closure {
public:
    Closure(const Cosmology& C, const PowerSpectrum& P_i, real z_i, real z, int Neta, int Nk = 1000, real kcut = 10.);
    ~Closure();

    static real U(real x);
...
//    real f2_11(real k, real q, real x) const;
...
//    real f2_22(real k, real q, real x) const;
    ...
    struct ThirdOrderIntegral : public MonteCarloIntegral {
        ThirdOrderIntegral(const Closure& closure);
